...
class ChatOrchestrator:
    ...
    graph_factory: AgentGraphFactory = field(default_factory=AgentGraphFactory)
    _graph: Any = field(init=False, repr=False)

    def __post_init__(self):

        self._graph = self.graph_factory.build()
    ...
    def run_turn(
        ...
        *,
        ...
        round_start = len(messages) - 1
        result: AgentState = self._graph.invoke(
            {"messages": messages, "verbose": verbose}
        )

        final_messages = result["messages"]
        reply = self._extract_reply(final_messages)
        delta = final_messages[round_start:]
        return reply, delta

    def _extract_reply(self, messages: list[dict]) -> str:
        last = messages[-1]
...
def run_turn(
    ...
    *,
    ...
    return default_orchestrator.run_turn(
        user_input,
        history,
        verbose=verbose,
    )
